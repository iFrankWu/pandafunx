...
namespace pandafun
{
    ...
    // "pandafun.io|TYPE|count|memo"
    // TYPE now: P(anda), (boo)K, C(andle), T(ea)
    // "pandafun.io|B|eosaccount|pandaid|memo"
    // TYPE now: B(uy)
    void apply_transfer( const pandafun::transfer& transfer_msg )
	{
		if (transfer_msg.from == code_account || transfer_msg.to != code_account)
        {
			return;
		}
        eosio_assert( transfer_msg.quantity.symbol == S(4,EOS) , "Only EOS token allowed.\n" );
        eosio_assert( transfer_msg.quantity.is_valid(), "Invalid token transfer\n" );
        eosio_assert( transfer_msg.quantity.amount > 0, "Quantity must be positive.\n" );
        auto typestart = transfer_msg.memo.find('|');
        // Parse the type
        if (typestart == std::string::npos)
        {
            print("No | for type start found in memo, malformed memo.\n");
            return;
        }
        auto countstart = transfer_msg.memo.find('|', typestart+1);
        if (countstart == std::string::npos)
        {
            print("No | for count start found in memo, malformed memo.\n");
            return;
        }
        if (countstart - typestart != 2)
        {
            print("action's type's length is illegal, ignored.");
            return;
        }
        char type = (char)transfer_msg.memo[typestart+1];
        if (type == 'P' || type == 'K' || type == 'C' || type == 'T')
        {
            auto countend = transfer_msg.memo.find('|', countstart+1);
            if (countend == std::string::npos)
            {
                print("No | for count end found in memo, malformed memo.\n");
                return;
            }
            auto num_str = transfer_msg.memo.substr(countstart+1, countend-countstart-1);
            if (num_str.size() <= 0 || num_str.size() > 9)
            {
                print("count's length is illegal, ignored.");
                return;
            }
            int64_t count = 0;
            for(int i=0; i < num_str.size(); i++)
            {
                ...
                {
                    print("Non-number char in count, ignored.");
                    return;
                }
                count = count * 10 + (num_str[i] - '0');
            }
            int64_t total = 0;
            if (type == 'K' || type == 'C' || type == 'T')
            {
                if (count >= discount_num)
                {
                    total = count * props_price / 10 * 8;
                }
                else
                {
                    total = count * props_price;
                }
            }
            else
            {
                total = count * panda_price;
            }
            if (transfer_msg.quantity.amount < total)
            {
                print("quantity in transfer is less than the needed: ", total);
                return;
            }

            if (type == 'P')
            {
                pandafun::transfers msg = {transfer_msg.from, (int32_t)count, "pandafun.io: buy panda" };
                action(
                   permission_level{ auth_account, N(active) },
                   code_account, N(transfers),
                   msg
                ).send();
            }
            else if (type == 'C')
            {
                pandafun::buycandles msg = {transfer_msg.from, count, 0};
                action(
                   permission_level{ auth_account, N(active) },
                   code_account, N(buycandles),
                   //std::make_tuple(transfer_msg.from, count, 0)
                   msg
                ).send();
            }
            else if (type == 'K')
            {
                pandafun::buybooks msg = {transfer_msg.from, count, 0};
                action(
                   permission_level{ auth_account, N(active) },
                   code_account, N(buybooks),
                   msg
                ).send();
            }
            else if (type == 'T')
            {
                pandafun::buytea msg = {transfer_msg.from, count, 0};
                action(
                   permission_level{ auth_account, N(active) },
                   code_account, N(buytea),
                   msg
                ).send();
            }
            action(
               ...
               std::make_tuple(code_account, N(pandafunhot1), transfer_msg.quantity, std::string("pandafun.io: buy props"))
            ).send();
        }
        else if (type == 'B')
        {
            auto accountend = transfer_msg.memo.find('|', countstart+1);
            auto pandaidend = transfer_msg.memo.find('|', accountend+1);
            if (countstart == std::string::npos || accountend == std::string::npos || pandaidend == std::string::npos)
            {
                print("No enough | found in memo, malformed memo.\n");
                return;
            }
            auto seller_str = transfer_msg.memo.substr(countstart+1, accountend-countstart-1);
            if (seller_str.size() <= 0 || seller_str.size() > 12)
            {
                print("seller's name length is illegal, ignored.");
                return;
            }
            for(int i=0; i < seller_str.size(); i++)
            {
                if( ! ( (seller_str[i] >= '0' && seller_str[i] <= '5') ||
                        (seller_str[i] >= 'a' && seller_str[i] <= 'z') ||
                        (seller_str[i] == '.') ) )
                {
                    print("seller's name contains illegal char, ignored.");
                    return;
                }
            }
            const account_name seller = string_to_name(seller_str.c_str());
            auto pandaid_str = transfer_msg.memo.substr(accountend+1, pandaidend-accountend-1);
            if (pandaid_str.size() <= 0 || pandaid_str.size() > 18)
            {
                print("Illegal panda id, ignored.");
                return;
            }
            uint64_t pandaid = 0;
            for(int i=0; i < pandaid_str.size(); i++)
            {
                if(pandaid_str[i] < '0' || pandaid_str[i] > '9')
                    return;
                pandaid = pandaid * 10 + (pandaid_str[i] - '0');
            }
            pandafun::buy msg = {seller, transfer_msg.from, pandaid, transfer_msg.quantity, "pandafun.io: buy panda from player" };
            action(
               permission_level{ auth_account, N(active) },
               code_account, N(buy),
               msg
            ).send();
            auto amount = transfer_msg.quantity * 95 / 100;
            auto fee = transfer_msg.quantity - amount;
            action(
               permission_level{ code_account, N(active) },
               N(eosio.token), N(transfer),
               std::make_tuple(code_account, seller, amount, std::string("pandafun.io: selling panda income"))
            ).send();
            action(
               permission_level{ code_account, N(active) },
               N(eosio.token), N(transfer),
               std::make_tuple(code_account, N(pandafunhot1), fee, std::string("pandafun.io: fee from panda deal"))
            ).send();
        }
        else
        {
            print("Illegal/Unknown method type, ignored.\n");
            return;
        }
    ...
    void apply_pray( const pandafun::pray& pray_msg )
    {
        ...
        if (candlesitr == candles.end())    // a new day
        {
            ...
            // add yesterday's value to the statistics, subtract the 4th day's value before if exists
            ...
            // we haven't counted the current day's usage into n3
            for (int i = 0; i < 4; i++)
            {
                if (reverseitr != candles.rend())
                    reverseitr++;
            ...
            // only can erase from the begin(), cannot from rbegin()
            if (reverseitr != candles.rend())
            {
                auto stoptimestamp = reverseitr->timestamp;
                auto citr = candles.begin();
                while (citr != candles.end() && citr->timestamp <= stoptimestamp)
                {
                    configs.modify(cfgitr, code_account, [&](auto &rec)
                    {
                        rec.candles7days = cfgitr->candles7days - citr->number;
                    });
                    //citr++;
                    citr = candles.erase(citr);
                }
            }
        }
        else
    ...
    void apply_upgrade( const pandafun::upgrade& upgrade_msg )
    {
        ...
        if (booksitr == books.end())    // a new day
        {
            ...
            {
                auto stoptimestamp = reverseitr->timestamp;
                auto citr = books.begin();
                while (citr != books.end() && citr->timestamp <= stoptimestamp)
                {
                    configs.modify(cfgitr, code_account, [&](auto &rec)
                    {
                        rec.upgrade7days = cfgitr->upgrade7days - citr->number;
                    });
                    citr = books.erase(citr);
                }
            }
        }
        else
    ...
    void apply_transform( const pandafun::transform& transform_msg )
    {
        ...
        if (teaitr == tea.end())    // a new day
        {
            ...
            if (reverseitr != tea.rend())
            {
                auto stoptimestamp = reverseitr->timestamp;
                auto citr = tea.begin();
                while (citr != tea.end() && citr->timestamp <= stoptimestamp)
                {
                    configs.modify(cfgitr, code_account, [&](auto &rec)
                    {
                        rec.transform7days = cfgitr->transform7days - citr->number;
                    });
                    citr = tea.erase(citr);
                }
            }
     ...
     * @param login_msg
     */
    void apply_login(const pandafun::login &login_msg){
        if (login_msg.eos_account == code_account || login_msg.eos_account == auth_account) {
            return;
        }

        eosio_assert(is_account(login_msg.eos_account), "account invalid");

        //登录验证授权
        require_auth(login_msg.eos_account);

        signup_type signup_table(code_account, code_account);

        auto itr = signup_table.find(login_msg.eos_account);
        eosio_assert(itr != signup_table.end(), "account not exist");
    }
    /***
     * 签到
     * @param signin_msg
     */
    void apply_signin(const pandafun::signin &signin_msg){
        if (signin_msg.eos_account == code_account || signin_msg.eos_account == auth_account) {
            return;
        }

        eosio_assert(is_account(signin_msg.eos_account), "account invalid");

        //登录验证授权
        require_auth(signin_msg.eos_account);

        signup_type signup_table(code_account, code_account);

        auto itr = signup_table.find(signin_msg.eos_account);
        eosio_assert(itr != signup_table.end(), "account not exist");
    }
...
extern "C"
{
    void apply( uint64_t receiver, uint64_t code, uint64_t action )
    {
        ...
        else if( code == code_account)
        {
            ...
            else if( action == N(login) )
                pandafun::apply_login(unpack_action_data<login>());
            else if( action == N(signin) )
                pandafun::apply_signin(unpack_action_data<signin>());
            else
